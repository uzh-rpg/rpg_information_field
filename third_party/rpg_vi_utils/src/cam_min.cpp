...
const std::string PinholeCam::kGeoHeader = "fx fy cx cy img_w img_h";
...
void PinholeCam::computeBearingVectors()
{
  if (pixel_bearing_computed_)
  {
    return;
  }
  int n_bearings = w_ * h_;
  bearings_at_pixels_.resize(Eigen::NoChange, n_bearings);

  Eigen::Matrix2Xd us;
  us.resize(Eigen::NoChange, n_bearings);
  for (int x = 0; x < w_; x++)
  {
    for (int y = 0; y < h_; y++)
    {
      int flat_idx = pixelCoordToFlatIdx(x, y);
      us.col(flat_idx)(0) = static_cast<double>(x);
      us.col(flat_idx)(1) = static_cast<double>(y);
    }
  }
  backproject3dBatch(us, &bearings_at_pixels_);

  pixel_bearing_computed_ = true;
}

PinholeCamPtr PinholeCam::createTestCam()
{
  rpg::Pose Tbc;
  Tbc.setIdentity();
  return std::make_shared<PinholeCam>(
      std::vector<double>{ 320.0, 320.0, 320.0, 240.0, 640.0, 480.0 }, Tbc);
}
                            ...
                            const std::string& abs_Tbc) const
{
  ...
  geo_params(0, 4) = w_;
  geo_params(0, 5) = h_;
  rpg::save(abs_cam_geo, geo_params, rpg::EigenSpaceSeparatedFmt, kGeoHeader);
...
void NCamera::projectBatchWithIds(const StatesVec& states_vec,
                                  const PinholeCamVec& cam_vec, const Map& map,
