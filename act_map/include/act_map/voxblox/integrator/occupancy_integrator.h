...
#include "act_map/voxblox/core/block_hash.h"
#include "act_map/voxblox/core/layer.h"
#include "act_map/voxblox/core/voxel.h"
#include "act_map/voxblox/integrator/integrator_utils.h"
#include "act_map/voxblox/utils/timing.h"
...
class OccupancyIntegrator {
 public:
  ...
  updateOccupancyVoxel(bool occupied, OccupancyVoxel* occ_voxel) {
    ...
    bool prev_occupied = occ_voxel->probability_log > min_occupancy_log_;
    ...
    if ((log_odds_update >= 0 &&
      ...
      return VoxUpdateRes::kUnchanged;
    }
    ...
    bool after_occupied = occ_voxel->probability_log > min_occupancy_log_;

    if (prev_occupied && !after_occupied)
    {
      return VoxUpdateRes::kOccToFree;
    }
    if(!prev_occupied && after_occupied)
    {
      return VoxUpdateRes::kFreeToOcc;
    }
    return VoxUpdateRes::kUnchanged;
  }
  ...
  void integratePointCloud(const Transformation& T_G_C,
                           ...
                           HierarchicalIndexMap* del_pts_list=nullptr,
                           HierarchicalIndexMap* add_pts_list=nullptr) {
    ...
    for (const GlobalIndex& global_voxel_idx : free_cells) {
      ...
      VoxUpdateRes res = updateOccupancyVoxel(occupied, &occ_voxel);
      if (res == VoxUpdateRes::kOccToFree && del_pts_list)
      {
        (*del_pts_list)[block_idx].push_back(local_voxel_idx);
      }
      if (res == VoxUpdateRes::kFreeToOcc && add_pts_list)
      {
        (*add_pts_list)[block_idx].push_back(local_voxel_idx);
      }
    }
    block.reset();
