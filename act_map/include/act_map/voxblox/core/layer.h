...
class Layer {
 public:
  ...
  typedef std::shared_ptr<const Layer<VoxelType>> ConstPtr;
  ...
  size_t getNumberOfAllocatedVoxels() const
  {
    return getNumberOfAllocatedBlocks() * std::pow(voxels_per_side(), 3);
  }
...
#include "act_map/voxblox/core/layer_inl.h"
