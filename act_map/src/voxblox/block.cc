...
void Block<OccupancyVoxel>::deserializeFromIntegers(
  ...
  for (size_t voxel_idx = 0u, data_idx = 0u;
       ...
       ++voxel_idx, data_idx += kNumDataPacketsPerVoxel) {
    const uint32_t bytes_1 = data[data_idx];
    std::array<uint32_t, 3> dir_bytes;
    for (size_t idx = 0; idx < 3; idx++)
    {
      dir_bytes[idx] = data[data_idx + 1u + idx];
    }
    const uint32_t bytes_2 = data[data_idx + 4u];

    OccupancyVoxel& voxel = voxels_[voxel_idx];
    memcpy(&(voxel.probability_log), &bytes_1, sizeof(bytes_1));
    for (int idx = 0; idx < 3; idx ++)
    {
      memcpy(&(voxel.aver_view_from_pt[idx]),
             &(dir_bytes[static_cast<size_t>(idx)]), sizeof(uint32_t));
    }
...
template <>
void Block<TsdfVoxel>::serializeToIntegers(
...
void Block<OccupancyVoxel>::serializeToIntegers(
  ...
  for (size_t voxel_idx = 0u; voxel_idx < num_voxels_; ++voxel_idx) {
    ...
    for (int idx = 0; idx < 3; idx++)
    {
      const uint32_t* dir_ele_ptr =
          reinterpret_cast<const uint32_t*>(&(voxel.aver_view_from_pt(idx)));
      data->push_back(*dir_ele_ptr);
    }
...
template <>
void Block<EsdfVoxel>::serializeToIntegers(
    std::vector<uint64_t>* data) const {
