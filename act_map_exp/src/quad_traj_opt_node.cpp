...
RPG_COMMON_MAIN
{
  ...
  {
    act_map_exp::QuadTrajOpt<act_map::GPTraceVoxel> planner(nh, pnh);
    ros::spin();
  }
  else if (FLAGS_kernel_type == "GPInfo")
  {
    act_map_exp::QuadTrajOpt<act_map::GPInfoVoxel> planner(nh, pnh);
    ros::spin();
  }
  else if (FLAGS_kernel_type == "QTrace")
  {
    act_map_exp::QuadTrajOpt<act_map::QuadTraceVoxel> planner(nh, pnh);
    ros::spin();
  }
  else if (FLAGS_kernel_type == "QInfo")
  {
    act_map_exp::QuadTrajOpt<act_map::QuadInfoVoxel> planner(nh, pnh);
    ros::spin();
  }
  else
  {
